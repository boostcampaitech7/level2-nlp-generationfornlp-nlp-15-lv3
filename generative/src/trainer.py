...
def get_model(config):
    ...
    if config['use_gptq']:
        ...
        try:
            ...
            if 'adapter_name' in kwargs: del kwargs['adapter_name']
            model = AutoModelForCausalLM.from_pretrained(
                config["model_name"],
                **kwargs
            )
    ...
    def _init_trainer(self):
        ...
        if self.use_lora:
            ...
            if self.config['use_merge']:
                weights = [0.5, 0.5]
                density = 0.5
                self.model.load_adapter(self.config['merge_model_name'], adapter_name="adapter")
                self.model.add_weighted_adapter(["base", "adapter"], weights, "merge", combination_type="dare_ties", density=density)
